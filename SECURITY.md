...
| F-2 | **Dashboard auth via `?key=` query token** (`server.py`). Tokens in query strings can leak to proxy/access logs and `Referer`. | 🟡 Low | Open | Single-user, read-only dashboard. Mitigation: move to an `Authorization` header or signed cookie. |
