...
> Lê sinais de atenção na **CoinMarketCap**, decide, e assina/executa swaps com autocustódia local via **Trust Wallet Agent Kit**. O capital "vai ao mercado e volta" — como um bumerangue.
...
3. **Filtro 3 — TWAK** (`vault/twak_executor.py`): assina swap localmente, stop-loss + trailing, e devolução de capital (boomerang).
...
🚧 Em desenvolvimento. Ver `memory/boomerang-ai-plano-dev.md` (no diretório de sessão do Claude) para o plano faseado.
