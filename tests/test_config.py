...
def test_sizing_defaults(cfg):
    # Effective base size follows the user setting (25% — raised so trades clear the fixed fee
    # floor); dev_safety base (5%) is the fallback; max_position_pct caps any conviction-scaled bet.
    assert cfg.dev_safety["position_size_pct"] == 5.0
    assert cfg.position_size_pct == 25.0
    assert cfg.user_position_size_pct == 25.0
